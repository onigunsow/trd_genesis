...
# 2026-08-16: 고가주 1주 예외. SK하이닉스 1주 159만원 > 단일 주문 10%(100만)·종목당 15%
# (150만) of 자본 1,000만 → 결정 페르소나가 "판단이 아니라 한도" 로 매수 불가 판정
# (8/14 rationale). 최소 거래단위가 1주인 시장에서 자본이 작으면 시총 상위주가 통째로
# 유니버스에서 빠지는 왜곡. 미보유 종목의 정확히 1주 매수는 이 비율까지 허용한다.
...
# 2026-08-27: 진입 바닥선. 프롬프트는 "confidence 가 0.50 이하면 진입 자체를 하지
# 않는다" 를 하드 룰로 적어왔는데 코드에 그걸 읽는 줄이 없었다 — risk/ 전체와
# 오케스트레이터에서 confidence 참조 0건. 그 결과 페르소나가 0.45 라고 써놓고
# 그 종목을 산 게 8건이다. 자기가 내놓은 숫자를 자기가 어기면 confidence 필드
# 자체가 무의미해진다. 값의 최적성과 별개로 계약은 지켜져야 한다.
# 임계 자체는 아직 근거가 없다 — 옛 정의 표본에서 0.50 이하가 오히려 성적이
# 좋았으나 n=8 이고 정의가 8/15 에 바뀌었다. 새 정의 표본이 20거래일을 채우면
# 대시보드 '진입 근거 귀속' 이 판정한다. 그때까지는 이 값을 튜닝 손잡이로 둔다.
DECISION_CONFIDENCE_FLOOR: Final[float] = float(
    os.getenv("DECISION_CONFIDENCE_FLOOR", "0.50")
)
