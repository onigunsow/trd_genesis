...
HOLD_REASON_KEYWORDS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("손실 재진입", ("재진입", "손실 청산")),
    # 2026-08-27: 종전 키워드가 맨 "한도" 라 "한도 5종 전항목 준수" 처럼 **지켰다**
    # 고 적은 근거까지 한도 위반으로 분류했다. 실측에서 이 라벨 8건 중 7건이
    # 035420 이었는데 전부 "수치상 위반은 없으나" 로 시작하는 근거였다 —
    # 지표가 뜻을 뒤집어 읽고 있었다. 위반을 가리키는 표현만 잡는다.
    ("한도", ("한도 초과", "한도 소진", "여력 소진", "저촉", "breach", "80% 이상")),
    ("수치 불일치", ("불일치", "실측")),
    ("순매도", ("순매도",)),
)
